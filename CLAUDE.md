...
| 037 | `037_player_injured.sql` | **`is_injured boolean not null default false` added to `match_players` — set on the OUTGOING row when a player is subbed out due to injury (tournament matches only). Drives the "Injured" tag on the scorecard. Cleared on swap-back.** |
| 038 | `038_fix_record_delivery_double_count.sql` | **ROOT-CAUSE FIX: `record_delivery` RPC seeded new batting/bowling/fielding rows with the first ball's values via `INSERT ... ON CONFLICT DO NOTHING`, then ran an unconditional `UPDATE col = col + value` — double-counting every player's FIRST ball (+1 ball + first-ball runs for each batsman; first ball for each bowler; first dismissal per fielder). Fixed by seeding the INSERT with ZEROS so only the UPDATE counts. Deliveries + innings totals were always correct.** |
| 039 | `039_reconcile_scorecards_and_stats.sql` | **ONE-TIME DATA CORRECTION (run after 038, back up first). Part 1: recomputes all `batting/bowling/fielding_scorecards` aggregates from the deliveries (source of truth). Part 2: wipes + replays `player_career_stats`/`player_tournament_stats` via `update_player_career_stats` over every completed innings, re-runs `increment_matches_played`, and preserves client-set `bowl_hat_tricks`. Idempotent.** |
| 040 | `040_match_date.sql` | **`match_date date` added to `matches` — the actual "played on" date (created_at = when the match was SET UP, which is wrong when set up and played on different days). Captured client-side in `matchService.startMatch` when a match first goes live; UI shows `match_date` and falls back to `created_at` via `matchDateValue(match)`. Backfills the K7 Trophy Season 7 matches to 27 Jun 2026. ⚠ Apply BEFORE deploying the code — `startMatch` writes `match_date`, so the column must exist.** |
...
- Props: `{ batsmanId: string, inningsIds?: string[] }` (`batsmanId` = the player id; the panel covers them as **both batsman and bowler**)
- **Two toggle views:** "vs Bowlers" (batting H2H via `getHeadToHeadAll` → B/R/**Dis**) and "vs Batsmen" (bowling H2H via `getBowlingHeadToHeadAll` → B/R/**W**). Both fetched in parallel; defaults to whichever side has data (a pure bowler opens on Bowling). A toggle is disabled when its side has no data.
- Tapping a row drills into a 3×2 stat grid — batting: balls/runs/dismissed/SR/dot%/4s-6s; bowling: balls/runs/wickets/econ/dot%/4s-6s
- **Why the bowling view matters:** previously the tab showed only the player AS A BATSMAN, so a bowler's wickets were invisible (e.g. Santosh's 7 wkts split Naveen×4/Sarath×2/Sujith×1 weren't shown; the "Dis" column only counted the 2 times he was out batting). Now both perspectives are available.
...
- `getHeadToHeadAll(batsmanId, inningsIds)`: player AS BATSMAN vs each bowler (deliveries-based) — B/R/dismissals-suffered/SR/dot%/4s/6s. `getBowlingHeadToHeadAll(bowlerId, inningsIds)`: player AS BOWLER vs each batsman — B(legal)/R-conceded(byes excluded)/**wickets-taken**/econ/dot%. Both filter by `inningsIds` when provided (series view), else all matches (career).
...
- `getH2HTopPerformers(matchIds)`: innings IDs → parallel batting + bowling scorecards → aggregate by player → top 3 each. **Reads `batting_scorecards.runs` and `bowling_scorecards.wickets`** (NOT `bat_runs`/`bowl_wickets` — those are `player_career_stats` columns; the old names 400'd and the error was swallowed → empty Top Performers). Batting join uses `players!player_id` (batting_scorecards has 3 player FKs). Throws on query error (no longer swallowed).
...
- `getSeriesPlayerMotmBreakdown(tournamentId, playerId)`: gathers the player's batting/bowling/fielding scorecards across the tournament's completed matches (server-side `.eq('player_id')` filter), returns `calcMotmBreakdown(...)` + `{ matches }` count. Powers the Man-of-the-Series share card. Returns `{ total:0, groups:[], matches }` gracefully when there are no matches/scorecards.
...
- **`calcMotmBreakdown(playerId, battingCards, bowlingCards, fieldingCards)`** — itemised decomposition of `calcMotmScore`: `{ total, groups: [{ title, subtotal, items: [{ label, detail, pts }] }] }`, grouped BATTING/BOWLING/FIELDING (empty groups dropped, duck shown as negative). **Sum of all item pts === calcMotmScore** (test-enforced). Powers the Man-of-the-Series share card's points breakdown.
- **`calcMvpScore(stats)`** — weighted career/series MVP aggregate: `runs×0.5 + wickets×20 + fours×1 + sixes×2 + thirties×5 + fifties×10 + hundreds×25 + catches×5 + stumpings×5 + run_outs×3` (lives here, imported by `Leaderboard.jsx`).
- **`calcMvpBreakdown(stats)`** — itemised decomposition of `calcMvpScore`, same `{ total, groups }` shape as `calcMotmBreakdown` (sum === score, test-enforced). Powers the **MVP share card** points breakdown.
- **`buildScorecardsFromDeliveries(deliveries)`** — rebuilds `{ battingCards, bowlingCards, fieldingCards }` (calcMotmScore-shaped, same field names as stored rows incl. `is_not_out`) from the immutable ball-by-ball log. Use when the stored `*_scorecards` aggregates may have **drifted** from deliveries. Used by `MatchSummary.jsx` so the Summary tab's MoTM card + top performers + override points always match the Scorecard tab and the team total.
...
- **MVP formula:** `runs×0.5 + wickets×20 + fours×1 + sixes×2 + thirties×5 + fifties×10 + hundreds×25 + catches×5 + stumpings×5 + run_outs×3` — `calcMvpScore` now lives in `cricketUtils.js` (imported here); `MVP_FORMULA` array in this file is the modal explainer only.
- **MVP shareable card (top 3 only):** in the MVP tab, the top-3 player-name cells show a green `Share2` icon and open `MvpCardSheet` (rank 4+ keep profile navigation via `mvpPlayerCell(row, rank)`). `mvpCard` state holds `{ row, rank }`; cleared on tab change. The card reflects whatever view is active (career or series-filtered stats).

### `src/components/player/MvpCardSheet.jsx`
- Props: `{ open, onClose, player, rank, mvpScore, stats }`
- Mirrors `PlayerMatchCardSheet`'s generate→preview→share flow (Satori → 1080×1350 PNG via `generateMvpCard`); Web Share API → clipboard → download fallback. No SR chart.
- `generateMvpCard({ player, rank, stats, asOf })` in `generateShareCard.jsx` — **points-breakdown design mirroring the MoS card**: rank-coloured band (gold/silver/bronze via `mvpRankMeta(rank)`) + identity (avatar + name + role + N matches) + "HOW THE POINTS ADD UP" (`calcMvpBreakdown` grouped line items + subtotals) + rank-coloured **MVP POINTS** TOTAL box + footer with **"AS OF \<date\>"** stamp (`asOf` defaults to `formatCardDate()` = today, since MVP rankings stay dynamic). **Dynamic height** via `motmCardHeight` (shared). Fractional points (Runs 39×0.5 = +19.5) handled by `fmtPts`/`fmtNum`.
...
- **Satori gotchas (hard-won):**
  ...
  - **No WebP/HEIC/AVIF images** — Satori only decodes PNG/JPEG; a WebP photo makes `satori()` throw and breaks the whole card. `fetchPhotoAsDataUrl` decodes any fetched image via an `<img>` + canvas and re-encodes to **PNG** (downscaled to 512px), so any upload format works. Decode failure falls back to the initials avatar.
...
- **`battingCards`/`bowlingCards`/`fieldingCards` are derived from deliveries via `buildScorecardsFromDeliveries` (useMemo on `deliveriesMap`), NOT from `getScorecards`** — the stored `*_scorecards` aggregates can drift from the ball log during scoring (Match-05: every batsman's stored row was +1 ball). This keeps the MoTM card, top performers, and override points (`scoredPlayers` → `calcMotmScore`) consistent with the Scorecard tab + team total. **Root cause of the drift fixed in migration 038** (`record_delivery` double-counted each player's first ball); **historical data corrected by migration 039** (recompute scorecards from deliveries + rebuild career/tournament stats). After applying 038+039 the stored aggregates match the deliveries, so this client-side derivation becomes belt-and-suspenders.
- Loads innings list + deliveries (parallel) in one useEffect; `playerMeta` + `playersMap` built from matchPlayers
...
- **Man of the Series card**: shown when `tournament.man_of_series` is set (gold Trophy card). Two actions: **"Share Card"** (`openMosCard` → `getSeriesPlayerMotmBreakdown` → `MotmCardSheet`) and **"ⓘ How it's picked"** (`mosFormulaOpen` → `MotmFormulaSheet`). `getTournament` join extended to `man_of_series:man_of_series_id(id, name, photo_url, role)` so the card has avatar + role.
- **`src/components/tournament/MotmCardSheet.jsx`** — props `{ open, onClose, player, seriesName, breakdown }`; generates a Satori MoS card (`generateMotmSeriesCard`, **dynamic height** via `motmCardHeight` so the breakdown never clips) with the full points breakdown (HOW THE POINTS ADD UP → grouped line items → gold TOTAL box). **No "as of" date** — a series MoS is final once the tournament ends. Share via Web Share → clipboard → download. Won't generate until a `breakdown` is supplied. Avatar is the player photo (any upload format works — see WebP note in Satori gotchas).
...
- `getSeriesTournaments(seriesId)` — tournaments linked to this series, ordered by start_date asc. **Filters `is_deleted = false`** (soft-deleted tournaments must not show on the series page — `deleteTournament` only soft-deletes). Same filter added to `getSeriesPlayerStats`, `playerService.getSeriesMatchIds`, `playerService.getPlayerSeriesStats`.
- `getSeriesPlayerStats(seriesId)` — two-step: fetch (non-deleted) tournament IDs, fetch all `player_tournament_stats` for those IDs, aggregate per player (sum all fields, MAX for `bat_highest_score`, **best-of for `bowl_best_wickets`/`bowl_best_runs`** — most wickets, ties by fewest runs; also aggregates `bowl_matches`). Without these the series Bowling tab showed "undefined/undefined" in the BB column.
...
6. **Balls-faced ≠ legal deliveries.** A batsman's `balls_faced` counts every delivery **except wides** — a **no-ball IS a ball faced** (matches `batting_scorecards.balls_faced`, the `record_delivery` RPC, and migration 039's `sum(case when extra_type <> 'wide' then 1 end)`). `is_legal_delivery` is a *different* metric (excludes wides **and** no-balls) used only for over-counting (6 legal balls = 1 over). Don't audit `balls_faced` against `is_legal_delivery` — a batsman who faced a no-ball will look falsely "+1 drifted." Use `extra_type <> 'wide'`.
...
| `TournamentDetail.jsx` + `matchService.js` | Toss sheet when starting a tourney match was missing Last Man Standing, Super Over, and Free Hit on No Ball toggles — rules set in the normal flow were unavailable here | Added `rules` state to toss sheet; reset on `openToss`; rule checkboxes rendered below toss selectors; `startUpcomingMatch` extended with optional `rules` param merged into `updateMatch` payload |
...
| `Home.jsx` + `LiveScoring.jsx` + `StrikerIndicator.jsx` + `BallLog.jsx` + `Scorecard.jsx` + `MatchSummary.jsx` + `Leaderboard.jsx` + `ActivePlayerSpotlight.jsx` + `PlayerDrawAnimation.jsx` + `PlayerPoolManager.jsx` | Nickname showing instead of primary name in batting/bowling dropdowns, striker indicator, ball log, scorecards, match summary, leaderboard partnerships, and auction room — `displayName()` (`nickname \|\| name`) was used everywhere | Replaced every `displayName(p)` call with `p?.name` across all 10 files; removed all now-unused `displayName` imports. `displayName()` still exists in `cricketUtils.js` but is no longer called anywhere in the UI. Primary name always shown. |
...
| `useAuctionRoom.js` + `auctionStore.js` | Bid history not updating in real-time for non-auctioneer viewers — bids only appeared after page refresh | **Root cause:** `auction_bids INSERT` realtime events may not reach all clients if table replication isn't enabled. **Fix (2 parts):** (1) `auction_players UPDATE` handler now calls `_refreshBids(player.id)` whenever the active player has a bid (`current_bid != null`), so all viewers get fresh bids from DB via the already-working `auction_players` channel. (2) Polling starts on mount, stops automatically when all 4 channels confirm `SUBSCRIBED`, and restarts on `CHANNEL_ERROR`/`TIMED_OUT` — no longer runs permanently. |
| `useAuctionRoom.js` + `auctionStore.js` | Auction room auto-reloaded every 8s even with healthy realtime — `loadAuction` set `isLoading: true` on background polls, causing visible UI flash | Added `silent = false` param to `loadAuction` — silent polls skip the `isLoading` state change; polling stops cleanly once all 4 Supabase channels confirm `SUBSCRIBED` (`subscribedCount.current >= 4 → stopPolling()`); restarts automatically on channel error |
| `AuctionRoom.jsx` | Static sold card — no celebration animation when a player was dealt | Replaced static PNG preview with animated GIF: `generateSoldCardGif()` renders a 540×960 canvas at 15fps/3s (flat colors to avoid GIF dithering bands, `ctx.scale(1.5,1.5)` for high-DPI), encodes with `gifenc` library; `SoldCardModal` shows full-screen portal with progress bar during encoding |
| `AuctionRoom.jsx` | Wrong player's GIF appeared when deals happened in quick succession — previous slow GIF encode finished after next player's card opened | Added `soldCardGenRef = useRef(0)` generation counter; each `openSoldCard()` call increments it; GIF result is discarded if `soldCardGenRef.current !== genId` at resolve time |
| `AuctionRoom.jsx` | `setSoldCardUrl is not defined` crash when deal completed — `handleDeal` had its own inline card generation using removed state variables | `handleDeal` now routes through `openSoldCard(ap)` for all card generation; old `soldCardUrl`/`generatingCard` state replaced with `exportUrl`/`exportMimeType`/`recordingCard`/`recordingProgress` |
| `auctionService.js` | Deleting an auction left orphaned global teams in the registry — teams with `source_auction_id` had `ON DELETE SET NULL` so they survived, still appearing in tournament/match dropdowns | `deleteAuction(id)` now deletes `teams WHERE source_auction_id = id` first, then deletes the auction row; throws if either step fails without touching the other |
| `auctionService.js` | `raiseAuctioneerBid` made a redundant DB fetch for `auction_id` from `auction_players` after already fetching the team row (which also has `auction_id`) | Removed the extra `auction_players` fetch; `team.auction_id` used directly in the bid insert. |
...
| `PlayerSubSheet.jsx` + `matchStore.js` + `matchService.js` + `LiveScoring.jsx` + `Scorecard.jsx` + `MatchSummary.jsx` + `037_player_injured.sql` | No way to mark a mid-match sub as an injury replacement, or to show who was injured / who replaced whom | Migration 037 adds `is_injured` to `match_players`. PlayerSubSheet gains a tournament-only "Subbing out due to injury" toggle (Step 2); `swapPlayer(out,in,team,{injured})` sets `is_injured` on the outgoing row via new `setPlayerInjured` service fn; `swapBack` clears it (reversible). Scorecard + MatchSummary render red "Injured" + sky "Sub in for X" tags (X resolved from `subbed_out_player_id`). Tournament matches only (`tournamentMatch={!!match.tournament_id}`); regular matches keep the plain tactical-swap flow. Scorecard-only — share card unchanged |
| `MotmFormulaSheet.jsx` *(new)* + `TournamentDetail.jsx` | No in-app explainer for how Man of the Series is picked (only MVP had one) | New reusable `src/components/shared/MotmFormulaSheet.jsx` — gold-gradient modal (mirrors Leaderboard's `MvpFormulaModal`) listing the full `calcMotmScore` breakdown grouped Batting / Bowling / Fielding. MoS card on TournamentDetail gets an "ⓘ How it's picked" button (`mosFormulaOpen` state) that opens it. Title/footer are props so it can be reused for Man of the Match later. **Note:** this formula (impact-based: runs×1, wkts×25, SR/economy bonuses) is intentionally different from the Rankings **MVP** formula (`calcMvpScore`: runs×0.5, wkts×20, no SR/econ bonus) — the two can crown different players, by design |
| `MvpCardSheet.jsx` *(new)* + `generateShareCard.jsx` + `Leaderboard.jsx` | No shareable card for MVP leaderboard players (only per-match performance cards existed) | Added `generateMvpCard()` (Satori → 1080×1350 PNG) with rank-coloured band (gold/silver/bronze via `mvpRankMeta`), hero MVP score + 3×3 stat grid. New `MvpCardSheet` previews + shares it (Web Share → clipboard → download). In the Leaderboard MVP tab, **top-3** player names show a green share icon and open the card (`mvpPlayerCell(row, rank)`); rank 4+ keep profile navigation. `mvpCard` state cleared on tab change. Card footer carries an **"AS OF \<date\>"** stamp (`formatCardDate()`, generation date) so a shared snapshot is dated. Verified live: #1 Santosh (gold, 183.5), #2 Gokul (silver, 160.0) |
| `MotmCardSheet.jsx` *(new)* + `calcMotmBreakdown` (cricketUtils) + `getSeriesPlayerMotmBreakdown` (matchService) + `generateShareCard.jsx` + `TournamentDetail.jsx` + `tournamentService.js` | No shareable Man-of-the-Series card, and no way to see how the MoS points add up | `calcMotmBreakdown` decomposes `calcMotmScore` into itemised BATTING/BOWLING/FIELDING groups (sum === score, test-enforced). `getSeriesPlayerMotmBreakdown` gathers the MoS player's series scorecards → breakdown + matches. `generateMotmSeriesCard` (Satori, **dynamic height** via `motmCardHeight` to avoid clipping) renders gold band + identity + "HOW THE POINTS ADD UP" line items + gold TOTAL box (no "as of" date — MoS is final once the tournament ends). `MotmCardSheet` previews/shares it. MoS card on TournamentDetail gains a **"Share Card"** button (loads breakdown then opens the sheet). Verified live: Gokul 314 pts (BATTING 232 + BOWLING 30 + FIELDING 52) matches the stored MoS exactly |
| `generateShareCard.jsx` (`fetchPhotoAsDataUrl`) | Share cards (MoTM / MVP / MoS) threw "Could not generate card" for any player whose photo was a **WebP** (e.g. Viki) — Satori can't decode WebP/HEIC/AVIF | `fetchPhotoAsDataUrl` now decodes the fetched blob via an `<img>` (blob object URL → no canvas taint) and re-encodes to **PNG** (downscaled to 512px max), so every upload format is Satori-compatible. Decode failure still falls back to the initials avatar. Fixes all card types at once (central helper). Verified live: Viki's MVP card now renders with photo |
| `partnershipService.js` (`computePartnerships`) | Leaderboard Partnerships showed "**Unrecorded**" as the partner for every stand after the 1st wicket — the pair was reset to just the survivor on a wicket and the **incoming batsman was never added** | On every delivery, both the striker (`batsman_id`) and non-striker (`non_striker_before`) are added to the current pair `Set`, so the new batsman is captured on his first ball. Data was fine (`non_striker_before` 100% populated) — pure logic bug. Verified live: 0 "Unrecorded", all pairs show both names |
| `Leaderboard.jsx` + `generateShareCard.jsx` | Partnership rows had no player photos; the Man-of-the-Series card carried an "AS OF \<date\>" stamp even though a series MoS is final once the tournament ends | Added a small circular `PlayerAvatar` (size 22) beside **both** names in each partnership row. Removed the "as of" date from the MoS card footer (`generateMotmSeriesCard` no longer takes/uses `asOf`) — the MVP card keeps its date since MVP rankings stay dynamic. Both verified live |
| `cricketUtils.js` + `Leaderboard.jsx` + `generateShareCard.jsx` | MVP share card showed only a stat grid — no explanation of how the MVP points accumulated | Moved `calcMvpScore` into `cricketUtils.js`; added `calcMvpBreakdown` (itemised, total === score, test-enforced). **Redesigned the MVP card to mirror the MoS card**: rank band + identity + "HOW THE POINTS ADD UP" grouped line items (e.g. Runs 39×0.5 = +19.5, Wickets 7×20 = +140) + rank-coloured MVP POINTS TOTAL box, **dynamic height** via shared `motmCardHeight`, keeps the "AS OF \<date\>" stamp. Verified live: Santosh #1 = 183.5 (28.5+140+15), Viki #3 = 154 (41+100+13) |
| `cricketUtils.js` + `MatchSummary.jsx` | MoTM card showed different batting/bowling figures than the Scorecard tab (Match-05: card "25 (9)", scorecard "19 (8)") — the card read the stored `batting_scorecards` which had **drifted +1 ball per batsman** from the deliveries (deliveries tie to the team total of 50, scorecards do not) | New `buildScorecardsFromDeliveries(deliveries)` rebuilds calcMotmScore-shaped cards from the immutable ball log. `MatchSummary` now derives `battingCards`/`bowlingCards`/`fieldingCards` from it (useMemo on `deliveriesMap`) instead of `getScorecards`, so MoTM card + top performers + override points (`scoredPlayers`) all match the Scorecard tab. Verified live: MoTM now "19 (8) · 1/9 (1.0)", override "Viki — 70 pts" (Viki is the correct MoTM even with true numbers: 70 > Rahul 67) |
| `record_delivery` RPC (migration 038) + reconcile (039) | **ROOT CAUSE** of the scorecard drift: `record_delivery` seeded a new `*_scorecards` row with the first ball's values (`INSERT ... ON CONFLICT DO NOTHING`) then ran an unconditional `UPDATE col = col + value`, double-counting every player's first ball. Confirmed across all 9 Match-05 batsmen (+1 ball each, + first-ball runs: Viki six→+6, Rahul dot→+0, etc.) | Migration 038 seeds the INSERT with ZEROS so only the UPDATE counts (fixes batting, bowling, fielding). Migration 039 reconciles existing data: recompute scorecards from deliveries, rebuild career/tournament stats by replaying `update_player_career_stats` over completed innings + `increment_matches_played`, preserve `bowl_hat_tricks`. User applies both via Supabase SQL editor (manual-migration workflow) after a backup |
| `HeadToHeadPanel.jsx` + `playerService.js` (`getBowlingHeadToHeadAll`) | Player profile **H2H tab only showed the player as a BATSMAN** (vs bowlers, "Dis" = times he was out). For a bowler like Santosh (7 wkts) this was misleading — the "Dis" column showed 2 (his batting dismissals), and his wickets / who he dismissed weren't shown anywhere | Added `getBowlingHeadToHeadAll(bowlerId, inningsIds)` (player as bowler vs each batsman — wickets taken, runs conceded with byes excluded, legal balls). `HeadToHeadPanel` gained a **"vs Bowlers" / "vs Batsmen" toggle**; defaults to whichever side has data. Verified live: Santosh's bowling H2H shows Naveen×4 / Sarath×2 / Sujith×1 = 7 wkts (matches his stat) |
| `matchService.js` (`getH2HTopPerformers`) | Team Head-to-Head "Top Performers" was empty — the query selected `bat_runs`/`bowl_wickets`, which don't exist on the scorecard tables (they're `player_career_stats` columns). PostgREST returned a 400, and the code used `data \|\| []` so the error was swallowed → no batsmen/bowlers shown while the match clearly had performers | Select the real columns `runs`/`wickets`; aggregate `r.runs`/`r.wickets`; disambiguate the batting_scorecards→players join with `players!player_id` (3 FK refs); throw on `bat.error`/`bowl.error` instead of swallowing. Verified live: H2H now shows Gokul 120 / Santosh 7 wkts etc. |
| `040_match_date.sql` + `matchService.js` + `cricketUtils.js` + `MatchCard.jsx` + `FixtureList.jsx` + `Home.jsx` + `MatchHistoryTable.jsx` + `HeadToHead.jsx` | Match cards showed `created_at` (when the match row was SET UP), wrong when a tournament is set up one day and played another (e.g. set up 25 Jun, played 27 Jun) | Added `match_date` column (migration 040). `startMatch` captures the local play date (`toLocaleDateString('en-CA')`) the first time a match goes live. New `matchDateValue(match)` helper prefers `match_date` (parsed as LOCAL midnight to avoid a tz off-by-one) and falls back to `created_at`; used at all 5 date-display sites. Backfilled the K7 Season 7 matches to 27 Jun. **Migration 040 must be applied before deploying — `startMatch` writes the new column.** |
| `Matches.jsx` | All matches (incl. tournament matches) showed as a flat list — cluttered when a tournament added many matches | Matches now group by tournament into a collapsible `TournamentSection` (Trophy icon + tournament name + "N matches · M completed" + Live dot), with matches numbered 1..N **within** the tournament; standalone matches stay inline. `sections` useMemo over `listMatches()` (which joins `tournaments(name)`); a section anchors at its first match's position. Default state: **collapsed when the whole tournament is finished**, expanded when any match is live/upcoming |
| `seriesService.js` + `playerService.js` + `TournamentLeaderboard.jsx` + `cricketUtils.js` | Soft-deleted tournaments still appeared on the Series detail page (deletion "not working") — `getSeriesTournaments`/`getSeriesPlayerStats` (and `playerService` series queries) didn't filter `is_deleted = false` (only the main Tournaments list did). Also: React "unique key" warning in `TournamentLeaderboard` (rows keyed by `r.id` which is undefined on aggregated series rows), and BB column showed "undefined/undefined" (series aggregation dropped `bowl_best_wickets`/`bowl_best_runs`/`bowl_matches`) | Added `.eq('is_deleted', false)` to all four series tournament queries. Keyed `TournamentLeaderboard` rows by `r.player_id ?? r.id`. Aggregated best-bowling (most wickets, ties by fewest runs) + `bowl_matches` in `getSeriesPlayerStats`; made `formatBestFigures` return `—` for missing figures. Verified live: SIR K7 Trophy now shows 1 season, no console errors, BB column shows real figures |
...
| `AuctionRoom.jsx` | Sold card modal showed no captain indicator — captain auto-sold at base price looked identical to any other player | `openSoldCard` compares `soldTeam.captain_id === ap.player.user_id` to compute `isCaptain`; `SoldCardModal` renders an amber "⭐ Captain" pill badge below the player name when true. |
| `AuctioneerControls.jsx` | "Next Player" button was enabled while auction was paused — auctioneer could accidentally draw a player mid-pause | `disabled` condition extended to `!isLive` (already `true` when paused/completed); button re-enables automatically when status returns to `live`. |
| `PlayerAvatar.jsx` | Pool, Held, Sold, Deals sheets showed no player photos — component only accepted `name`/`photoUrl` as separate props, but auction sheets passed `player={ap.player}` (a shorthand object) + `size="sm"` (a named string); both were silently ignored | Added `player` shorthand prop (resolved `name ?? player?.name`, `photoUrl ?? player?.photo_url`); added `NAMED_SIZES` map (`xs=24, sm=32, md=44, lg=56, xl=80`) so string size tokens work; numeric sizes still pass through directly. |
| `playerService.js` (`getMatchHistory`) + `MatchHistoryTable.jsx` | Player profile **History** tab showed the Won/Lost badge on only some completed matches — the badge read `batting?.team \|\| bowling?.team`, but the scorecard `team` column is **unreliable** (`recomputeInnings` writes it back as `null` after any undo, and the RPC seeds it inconsistently). So matches whose scorecard `team` was null got **no badge**, while the 1–2 with it populated showed one | `getMatchHistory` already queries `match_players` for the player — now also selects `team` (the authoritative 1/2, always set) and attaches `playerTeam` to each history entry. `MatchHistoryTable` resolves the team as `playerTeam \|\| batting?.team \|\| bowling?.team` and passes it to `resultBadge`. Verified live: all 5 of Kamesh's completed K7 matches now show Won/Won/Won/Lost/Won (was 2). |
| `matchStore.js` + `Scoreboard.jsx` + `LiveScoring.jsx` | No way to change the over-limit mid-match — an 8-over match the teams agreed to shorten to 7/5 had to be re-configured or finished at the wrong count (and a manual "End Innings" before the configured limit produced a **blank result**, see win-condition note) | Added **dynamic overs adjustment**: `matchStore.setTotalOvers(n)` validates and writes `matches.total_overs`. **Guard:** cannot reduce below the overs already bowled in the **current** innings — any ball into over N+1 (`total_legal_balls > N*6`) blocks setting the limit to N; min allowed = `Math.max(1, Math.ceil(total_legal_balls/6))`. Undo the ball back to N.0 and the reduction is allowed. Throws `Error{ code:'OVERS_TOO_LOW', minOvers }`; also refuses during a super over and for non-integer/<1 values. `Scoreboard` gains an optional `onEditOvers` prop — when passed (and not a super over), the "Overs: X/Y" line becomes a pencil button. `LiveScoring` wires it to an "Adjust Overs" `BottomSheet` (− / + stepper bounded at the live minimum, Save disabled at no-change). **Reducing to the bowled count (e.g. set 5 at 5.0) makes `useWinCondition` see `oversCompleted` → the runs result computes correctly**, which also fixes the original "manual End Innings shows no winner" gap. Verified live on Match 04 (read-only): min 9 enforced, − disabled, increase enables Save, Cancel persisted nothing. |
| **Match 04 (K7 S7) dismissal correction — `deliveries` + `batting_scorecards` (manual SQL, no code change)** | Scorer swapped two Lion Kings dismissals: Sowmik logged as "run out (Viki)" and Yuvaraj as "c Dinesh Ravi Virat b Kamesh S" — reversed from reality (Kamesh bowled Sowmik, caught by Dinesh Ravi Virat; Yuvaraj run out by Viki) | Swapped `batsman_id`/`batsman_out_id` on the two wicket deliveries (over 2.4 ↔ 3.4, **both 0-run legal balls** → net-zero on ball & run counts) and the `dismissal_type`/`bowler_id`/`fielder_id` on both `batting_scorecards` rows. **Aggregates unaffected by design** — delivery `bowler_id`/`fielder_id` were left untouched, so Kamesh's wicket (1), Dinesh's catch (1), Viki's run-out (1) and every batsman's runs/balls stay identical; only the per-batsman *attribution* of each dismissal changes (which is exactly what makes the bowling/fielding H2H correct). Verified live post-fix: Sowmik now `c Dinesh Ravi Virat b Kamesh S, 16 (20)`, Yuvaraj `run out (Viki), 19 (6)`. **Technique for future single-dismissal corrections: only edit `deliveries.batsman_id/batsman_out_id`/wicket fields + the matching `batting_scorecards` dismissal columns; never touch delivery `bowler_id`/`fielder_id` unless the wicket truly changed bowler/fielder, and re-run migration 039 if any ball/run count actually shifts.** |
...
**54 test files, 788 tests — all passing:**

| File | What's tested |
|------|---------------|
| `src/lib/cricketUtils.test.js` | formatOvers, calcCRR/RRR/NRR, calcStrikeRate/Average/Economy, formatBestFigures (incl. **— for missing figures**), isMaiden, detectHatTrick, deriveRunType, applyStrikerSwap, calcWinByWickets/Runs, round, fmt, computeBadges (all 7 badges), calcMotmScore, pickMotm, **calcMotmBreakdown** (total === calcMotmScore for combined + multi-innings, correct batting line items, duck negative, empty groups omitted), **calcMvpScore** (weighted formula, empty/missing-field safe, fractional runs), **calcMvpBreakdown** (total === calcMvpScore, correct line items/details, empty groups omitted), **buildScorecardsFromDeliveries** (batting runs/balls/sixes with wide excluded + out status, bowling legal-balls/runs/wickets, fielding catch credit, maiden over, not-out batsman, feeds calcMotmScore correctly, empty input), **displayName** (nickname fallback, null/undefined safety), **matchDateValue** (prefers match_date parsed as local date, falls back to created_at, null when neither) |
...
| `src/components/scoring/BallLog.test.jsx` | Chip labels (dot/4/W/wd+N/nb+N/Nb/Lb), 24-chip limit, popover, resolveName fallback; **nickname guard** — popover shows `.name` not `.nickname` from joined object + matchPlayers fallback |
| `src/components/scoring/Scoreboard.test.jsx` | Regular innings: null guard, score display, overs/total_overs, match number, no SO label, RRR when target; Super over: ⚡ label, overs max=1, match number hidden, RRR with target, no RRR without target, balls-remaining uses 6-ball cap; **editable overs — `onEditOvers` makes the overs line a button that fires the callback, plain text when not provided, never editable during a super over** |
| `src/components/scoring/StrikerIndicator.test.jsx` | Striker/nonStriker render, Swap ends, retire callback, expand breakdown; **nickname guard** — shows `.name` not `.nickname` for striker + nonStriker |
...
| `src/services/matchService.test.js` | incrementMatchesPlayed, addSubPlayer, setPlayerActive, **setPlayerInjured** (updates is_injured, throws on error), getMatchNumber, createSuperOverInnings — inserts with is_super_over=true, passes target, throws on DB error; **startUpcomingMatch** — toss fields saved, battingTeam=1/2 from all 4 toss combinations, rule flags (last_man_standing/super_over_enabled/free_hit_on_no_ball) merged into payload, no extra keys when rules omitted, partial rules, status set to live, **captures match_date (today, YYYY-MM-DD) on first start**; **getH2HTopPerformers** — empty when no matchIds, aggregates top batsmen by `runs` + bowlers by `wickets` (correct scorecard columns), throws on scorecard query error; **getSeriesPlayerMotmBreakdown** — zeroed when no completed matches, aggregates scorecards with total matching the formula (201) + matches count, matches-count-only when player has no scorecards |
| `src/stores/matchStore.test.js` | swapPlayer — sequential order, no deactivate if insert throws, correct subbedOutPlayerId, store state, **no setPlayerInjured on tactical swap, marks injured when `{injured:true}`**; swapBack — finds linked sub (not any active sub), throws if no linked sub, store state, **clears injured flag on injured swap-back, does NOT call setPlayerInjured on tactical swap-back**; startSuperOverInnings — correct innings number, target, state reset (striker/nonStriker/bowler/deliveries), currentInnings updated, innings appended, keeper auto-set for bowling team; **setTotalOvers — increases & updates store, allows reduce to exactly N*6 bowled, rejects reduce below the in-progress over (5.1→5 throws `OVERS_TOO_LOW` minOvers 6), refuses during super over, rejects non-integer/<1** |
| `src/components/match/PlayerSubSheet.test.jsx` *(new)* | **Injured toggle** — hidden for non-tournament match, shown in Step 2 for tournament match; `onSwap` called with `{injured:false}` when off, `{injured:true}` when toggled on; **toggle resets** when going Back and picking a different player |
...
| `src/lib/generateShareCard.test.jsx` | getInitials, calcSR, calcEcon, dismissalText — pure helper functions for Satori card generation; **mvpRankMeta** — ordinal 1st/2nd/3rd, distinct title per rank, always returns gradient/accent/badge hex colors, graceful fallback for rank 4+; **formatCardDate** — "D Mon YYYY" format, ISO string input, invalid-date → '', defaults to today; **motmCardHeight** — min 1350, grows with group + item counts (clip-proof MoS card) |
| `src/components/tournament/MotmCardSheet.test.jsx` *(new)* | Null when closed; does NOT generate until a breakdown is supplied; generates with player/seriesName/breakdown; spinner → card image + Share button; error toast on failure |
| `src/components/player/MvpCardSheet.test.jsx` *(new)* | Renders null when closed; calls `generateMvpCard` with player/rank/score/stats on open; shows spinner then card image + "Share MVP Card" button; error toast on generation failure |
| `src/components/player/PlayerAvatar.test.jsx` | photo shown when photoUrl prop set; initials when no photo; numeric size applied as px; `player` shorthand — photo from player.photo_url, initials from player.name, explicit props override; named sizes sm=32/md=44; unknown size falls back to 40px; default 40px |
...
| `src/pages/Scorecard.test.jsx` | Realtime subscriptions — LIVE indicator shown/hidden on match UPDATE, innings UPDATE patches score in-place without reload, innings INSERT triggers reloadInnings, deliveries INSERT appends to deliveriesMap for known innings, ignores foreign innings_id, all 3 channels removed on unmount; **realtime delivery name resolution** — player name shown (not UUID) when realtime delivery has no joined player objects, resolved via playersMap; **nickname guard** — shows `.name` not `.nickname` from playersMap entry; **injury/sub tags** — "Injured" on injured player + "Sub in for X" on replacement, no tag for a normal player |
| `src/pages/MatchSummary.test.jsx` *(new)* | **Nickname guard** — batsman and bowler primary `.name` shown in SCORECARD tab, never `.nickname`, even when both are set on the player and delivery objects; **injury/sub tags** — "Injured" + "Sub in for X" shown in SCORECARD tab (batting + bowling rows), no tag for a normal player; **MoTM card derives from deliveries** — shows the delivery-derived line (18 off 3), not the corrupt stored `batting_scorecards` (25 off 9) |
| `src/services/seriesService.test.js` | listSeries, addSeries, deleteSeries — error propagation; **getSeriesTournaments** — filters `is_deleted = false`; getSeriesPlayerStats — empty when no tournaments, filters `is_deleted = false`, aggregates multiple rows per player (sums all numeric stats, MAX for bat_highest_score, **best-of for bowl_best_wickets/runs + bowl_matches**) |
| `src/services/playerService.test.js` (extended) | getSeriesMatchIds — empty when no tournaments, returns match IDs across all series tournaments; getPlayerSeriesStats — null when no data, aggregates all stat fields, MAX for best bowling figures; **getBowlingHeadToHeadAll** — aggregates wickets/runs/balls per batsman from the bowler side, byes excluded from runs conceded, sorts by wickets then balls, filters by inningsIds; **getMatchHistory** — attaches `playerTeam` from `match_players` (not the unreliable scorecard team), empty array when player has no matches |
| `src/components/player/MatchHistoryTable.test.jsx` *(new)* | Result badge — shows "Won" from `playerTeam` even when scorecard `team` is null (the bug), "Lost" for the losing team, falls back to scorecard team when `playerTeam` absent, no badge when neither team is known, "Tied" for a tie, no badge for a non-completed match, empty state |
...
| `src/components/shared/MotmFormulaSheet.test.jsx` *(new)* | Default "Man of the Series Formula" heading + Batting/Bowling/Fielding sections + representative rules (Wicket +25, Century +30, Run scored); custom title/footer (for MoTM reuse); onClose fires on close button |
| `src/stores/themeStore.test.js` | applyTheme: adds dark class, removes dark class, respects system light/dark preference |
| `src/services/partnershipService.test.js` | computePartnerships: empty input, single unbroken partnership, wide not counted as ball, broken partnership on wicket, sort by runs desc, multiple wickets, **incoming batsman captured after a wicket** (no null/"Unrecorded" partner) |
...
| `src/pages/Matches.test.jsx` *(new)* | **Tournament grouping** — tournament matches grouped under a collapsible `TournamentSection` (name + "N matches · M completed"); finished tournament collapsed by default, clicking header reveals matches numbered 1..N within the tournament; active tournament expanded by default; standalone (non-tournament) matches render flat |
| `src/pages/Leaderboard.test.jsx` | Renders Batting tab by default; switches to Partnerships tab without crash; **partnership rows** render both player names + avatars; switches to MVP tab without crash; **MVP card** — tapping a top-3 MVP name opens `MvpCardSheet` with the right player+rank; rank 4+ does NOT open the sheet |
...
| `src/pages/AuctionRoom.test.jsx` | Admin sees AuctioneerControls only; captain sees CaptainControls only; viewer sees neither; BudgetBars render per team; budget bar width reflects proportion; **BudgetBars captain avatar** — photo shown when captainPlayer has photo_url, initials shown when no photo, team name initials when captainPlayer is null; PassIndicator hidden/shown; Hold button animates when both passing; bid button disabled over budget; bid button enabled within budget; Pass button shows Passing state; waiting message when no active player; **delete button** — super admin (`kameshwaran26@gmail.com`) sees it, regular admin does not, non-admin does not; sold count chip visible; **SoldCardModal** — shows "Bought by" and "Final Price"; **(C) badge** — shown when player user_id matches team captain_id, hidden otherwise; **Deal guard** — single tap arms button ("✅ Confirm Deal?"), dealPlayer not called; tapping sold player opens modal instantly with no GIF generation |
| `src/components/auction/AuctioneerControls.test.jsx` | haptic feedback on chip/badge/clear; multi-tap chips — count, combine, decrement, clear, confirm raise, team advance, wrap, over-budget; default 10 bid increments; undo bid shown/hidden; **Next Player pause gate** — enabled when live+no active player, disabled when paused, disabled when player active, disabled when loading; Resume/Pause button visibility per status |
...
- Props: `{ open, onClose, match, matchPlayers, allPlayers, onSwap, onSwapBack, tournamentMatch }`
...
- **Injured toggle (tournament matches only):** when `tournamentMatch` is true, Step 2 shows a "Subbing out due to injury" toggle. `onSwap` is always called with a 4th arg `{ injured }` (off = tactical swap, no tag; on = injury → "Injured" tag on outgoing + "Sub in for X" on replacement). Toggle resets on team change and "← Back". Wired in `LiveScoring.jsx` via `tournamentMatch={!!match?.tournament_id}`.
...
- **Injury flow:** `swapPlayer(out, in, team, { injured })` — when `injured`, also calls `matchService.setPlayerInjured(out, true)` and patches the store row `is_injured: true`. `swapBack` clears it (`setPlayerInjured(benched, false)`) **only when the benched player was actually injured** (guarded to avoid a redundant write on tactical swap-backs). Scorecard tags driven by `is_injured` (red "Injured") + `is_substitute` + resolved `subbed_out_player_id` name (sky "Sub in for X") in both `Scorecard.jsx` (PlayerBadges — batting + bowling rows) and `MatchSummary.jsx` (inline badges in SCORECARD tab — batting **and** bowling rows). A chained sub who is later injured correctly shows BOTH tags. **Known limitation:** tags appear on the final scorecard/summary (fresh fetch); a live second-device viewer won't see a just-made sub until reload — same pre-existing limitation as C/WK badges (realtime doesn't refresh `match_players`).
...
- `src/pages/Leaderboard.jsx` — new "🤝 Partnerships" tab; lazy-loads on first visit; cards show rank, a small circular `PlayerAvatar` (size 22) + name for **both** batsmen, match context, runs + balls; `partnershipsLoaded` flag prevents re-fetching
...
- **`src/stores/auctionStore.js`** — Zustand store: `auction`, `teams`, `players`, `bids` state; `loadAuction(id, silent=false)` parallel fetches — `silent=true` skips `isLoading: true` so background polls cause no UI flash; `_patchPlayer`, `_appendBid`, `_removeBid`, `_refreshBids`, `_patchTeam` handlers for realtime events. `_refreshBids(playerRowId)` fetches fresh bids from DB and replaces store bids — called by `useAuctionRoom` on every active-player UPDATE so all viewers stay in sync regardless of `auction_bids` realtime delivery.
