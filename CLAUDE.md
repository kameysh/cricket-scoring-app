...
- **`src/stores/auctionStore.js`** — Zustand store: `auction`, `teams`, `players`, `bids` state; `loadAuction(id)` parallel fetches; `_patchPlayer`, `_appendBid`, `_removeBid`, `_refreshBids`, `_patchTeam` handlers for realtime events. `_refreshBids(playerRowId)` fetches fresh bids from DB and replaces store bids — called by `useAuctionRoom` on every active-player UPDATE so all viewers stay in sync regardless of `auction_bids` realtime delivery.
