...
export default function AcceptInvite() {
  ...
  useEffect(() => {
    // Supabase exchanges the URL hash token automatically on getSession
  ...
  async function onSubmit({ password }) {
    ...
    // Re-initialise auth store so role is loaded
    await init();
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#fafafb] dark:bg-ink-900">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-1">
          ...
          <h1 className="text-2xl font-bold text-ink-900 dark:text-white">Welcome!</h1>
          <p className="text-sm text-ink-500 dark:text-ink-400">Set a password to activate your account</p>
        </div>
